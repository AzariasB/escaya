...
describe('Statements - For', () => {
  ...
  for (const arg of [
    ...
    'for (const [arrow = () => {}] = []; x < 1; ) {}',
    'for (const [[x]] = [null]; ; ) {}',
    'for (const [{ x, y, z } = { x: 44, y: 55, z: 66 }] = []; x < 1; ) {}',
    'for (const [...x] = [1, 2, 3]; x < 1; ) {}',
    'for (const {} = null; ; ) {}',
    'for (const { fn = function () {}, xFn = function x() {} } = {}; x < 1; ) {}',
    'for (const { w: [x, y, z] = [4, 5, 6] } = {}; x < 1; ) {}',
    'for (const {...x} = { get v() { count++; return 2; } }; x < 1; ) {}',
    'for (let [x = 23] = [,]; x < 1; ) {}',
    'for (let [{ x, y, z } = { x: 44, y: 55, z: 66 }] = []; x < 1; ) {}',
    'for (let [{ u: v, w: x, y: z } = { u: 444, w: 555, y: 666 }] = [{ u: 777, w: 888, y: 999 }]; x < 1; ) {}',
    'for (let { gen = function* () {}, xGen = function* x() {} } = {}; x < 1; ) {}',
    'for (let { w: [x, y, z] = [4, 5, 6] } = { w: [7, undefined, ] }; x < 1; ) {}',
    'for (let { x: y } = { x: 23 }; x < 1; ) {}',
    'for (var [...x] = [1, 2, 3]; x < 1; ) {}',
    'for (var { w: [x, y, z] = [4, 5, 6] } = {}; x < 1; ) {}',
    'for (var { w: { x, y, z } = { x: 4, y: 5, z: 6 } } = { w: null }; x < 1; ) {}',
